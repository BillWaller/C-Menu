...
### **C-Menu** Test Curses Keys

Is a particular key not working for your project? NCurses Keys (or C-Keys)
provides an easy way to determine if there is a missing entry in your terminfo
database. After all, it couldn't be your code. (It could be my code.) Or, if you just don't remember the name of a particular key. Just start C-Keys and press the key. It will display the curses key name along with Octal, Decimal, and Hex codes. It's also a good way to identify which keys are not used by NCurses, so you can
claim them for your application.
...
As you can see, the view command on line 24 specifies the number of columns and lines (-C and -L respectively).

<img src="screenshots/main.m.png" />

:qrc="screenshots/nvim-log.png" alt="nvim log" title="nvim log" />
