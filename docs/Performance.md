...
| File Finder | Elapsed Time | Files Found |
| ----------- | ------------ | ----------- |
| find        | 0.86 seconds | 518,477     |
| fd          | 0.15 seconds | 518,477     |
| lf5         | 0.11 seconds | 518,477     |


The timings were obtained using the following script:
...
echo
echo lf5 -H -T9 "$directory"
/bin/time -o time.out ./lf5 -H -T9 "$directory" | wc -l >lf.out 2>/dev/null
grep -v "^Command" time.out
echo "lf found "$(cat lf.out)" files"
```

The benchmarks between fd and lf5 are too close to call a winner. That's okay
