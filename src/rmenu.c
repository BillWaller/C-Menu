...
int read_menu_file(menu_ *);
char get_command_type(char *);
void free_menu_line(line_ *);

int read_menu_file(menu_ *menu) {
    FILE *fp;
    ...
    int l;
    char *s, *d, *e, *p;
    if ((fp = fopen(menu->argv[1], "r")) == NULL) {
        strncpy(tmp_buf, menu->argv[1], BUFSIZ);
        strncat(tmp_buf, " not found", BUFSIZ);
        abend(-1, tmp_buf);
    }
    while ((fgets(in_buf, BUFSIZ, fp)) != NULL) {
        ...
        case ':':
            ...
            *d = '\0';
            ...
            if (l > menu->text_max_len)
                menu->text_max_len = l;
            if (menu->title == NULL) { /* in_buf -> Title */
                if (l + 5 > MAX_COLS)
                    l = MAX_COLS - 5;
                d = menu->title = (char *)malloc(MAX_COLS + 1);
                if (d == (char *)0) {
                    sprintf(tmp_str, "malloc(%d bytes) failed menu->title",
                            MAX_COLS + 1);
                    abend(-1, tmp_str);
                }
                e = d + l;
                ...
                *d = '\0';
                ...
                menu->line[menu->line_idx] = (line_ *)malloc(sizeof(line_));
                if (menu->line[menu->line_idx] == (line_ *)0) {
                    sprintf(tmp_str, "malloc(%ld bytes) failed menu->line[%d]",
                            sizeof(line_), menu->line_idx);
                    abend(-1, tmp_str);
                }
        ...
        case '\n':
            break;
        default:
            strncpy(tmp_buf, "unrecognized operator in ", BUFSIZ);
            strncat(tmp_buf, menu->argv[1], BUFSIZ);
            abend(-1, tmp_buf);
        }
    }
    fclose(fp);
...
void free_menu_line(line_ *line) {
    int j;
