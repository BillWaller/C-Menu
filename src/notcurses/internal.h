...
state for that window, and part of a pile.

Each pile has a total order along its z-axis.
Functions update these virtual planes over a series of API calls.
Eventually, notcurses_render() is called.
We then do a depth buffer blit of updated cells.

A cell is updated if the topmost plane including that cell updates it,
...
typedef struct ncplane {
    ...
    // pile.
    // a pile has a set of one or more root planes, all of them siblings.
...
/*
a pile is a collection of planes which will be rendered together.
piles are completely distinct with regards to thread-safety; one
can always operate concurrently on distinct piles (save rasterizing,
of course).

material from other piles will be blown away whenever a pile is
rasterized.

no ordering is meaningful between planes. when a new one is added,
or one is destroyed, the pilelock (in struct notcurses) is taken.
a pile is destroyed whenever its last plane is destroyed or
reparented away.

a pile contains a totally ordered list of piles (ordered on the z
axis) and a directed acyclic forest of bound planes, with each root
plane rooting a DAG.

the geometries are updated each time the pile is rendered.
until a pile is rendered, its geometries might be out-of-date with
regards to the terminal description in tcache.

when it is rendered again, the geometries will be updated, sprixels
will have their TAMs rebuilt (if necessary), and each root plane
will have its resize callback invoked (possibly invoking its
bound planes' resize callbacks in turn).

at context start, there is one pile (the standard pile), containing one
plane (the standard plane).

each ncplane holds a pointer to its pile.
*/
