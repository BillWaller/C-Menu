...
int pick_engine(Init *init) {
    ...
    do {
        ...
        rc = picker(init);
        if (rc == '\t') {
            flen = pick->win_width - 4;
            if (rc == '\t')
                rc = line_editor(pick->win, pick->win2, pick->chyron, flen,
                                 &field[0]);
        }
        if (rc == -1)
            break;
...
int picker(Init *init) {
    ...
    while (1) {
        ...
        switch (
            in_key) { /** 'q', or KEY_F(9) cancel selection and exit picker */
        case '\t':
            return in_key;
        case 'q':
        case 'Q':
 ...
 *  @ingroup pick_engine
   ...
   asterisk. Updates the chyron with page information at the bottom of the pick
   window. */
void display_matching_objects(Pick *pick, char *s) {
    int y, col, pidx;
    for (y = 0; y < pick->pg_lines; y++) {
        wmove(pick->win, y, 0);
        wclrtoeol(pick->win);
    ...
    for (col = 0; col < pick->tbl_cols; col++) {
        pick->x = col * (pick->tbl_col_width + 1) + 1;
        y = 0;
        while (y < pick->pg_lines && pidx++ < pick->obj_cnt) {
            if (pidx < pick->obj_cnt) {
                if (strstr(pick->object[pidx], s) != nullptr) {
                    if (pick->f_selected[pidx])
                        mvwaddstr(pick->win, y, pick->x - 1, "*");
                    mvwaddstr_fill(pick->win, y, pick->x, pick->object[pidx],
                                   pick->tbl_col_width);
    ...
    if (y < pick->pg_lines)
        wscrl(pick->win, y - pick->pg_lines);
    // compile_chyron(pick->chyron);
    // display_chyron(pick->win2, pick->chyron, 1, pick->chyron->l);
}
...
int line_editor(WINDOW *win, WINDOW *win2, Chyron *chyron, int flen,
    ...
    while (1) {
        if (in_key == 0) {
            if (accept_s[0] != '\0')
                display_matching_objects(pick, accept_s);
            e = accept_s + flen;
            s = accept_s + rtrim(accept_s);
            while (s != e)
                *s++ = ' ';
            *s = '\0';
