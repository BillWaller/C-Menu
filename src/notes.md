...
The 16-byte Notcurses nccell contains 32 bit field, (gcluster), which is can be extended, if necessary, through an EGC (Extended Grapheme Cluster) pool that is an architectural component of the Notcurses plane. This provides a robust way to handle Unicode characters and their attributes, but it does present some challenges when adapting existing code that was designed for NCurses.
