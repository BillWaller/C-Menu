...
unsigned int menu_engine(Init *init) {
    ...
    ui_tracked_sfc_box(menu->lines, menu->cols, menu->begy, menu->begx, menu->title);
    UiSurface *sfc = ui_surface[sfc_ptr];
    if (sfc == nullptr) {
        ssnprintf(tmp_str, MAXLEN - 1, "ui_tracked_sfc_box(%d, %d, %d, %d, %s) failed",
...
unsigned int menu_cmd_processor(Init *init) {
    ...
    while (1) {
        ...
        if (event.mouse_action == UIKEY_BUTTON1_CLICKED)
