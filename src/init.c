 ...
 */
void
mapp_initialization (Init *init, int argc, char **argv)
{
    char term[MAXLEN];
    char tmp_str[MAXLEN];
    char *e;
 ...
 */
int
parse_opt_args (Init *init, int argc, char **argv)
{
...
/** @brief parse the configuration file specified in init->minitrc and set
   ...
   newlines.
    ...
    Skip unknown keys. */
int
process_config_files (Init *init)
{
...
int
process_config_file (char *config_file_name, Init *init)
{
...
void
print_argp_doc (FILE *minitrc_fp, char *config_s, char *key)
{
    char comment[MAXLEN];
    comment[0] = '\0';
 ...
 */
bool
derive_file_spec (char *file_spec, char *dir, char *file_name)
{
    char ts[MAXLEN];
    char ts2[MAXLEN];
    char *e;
...
void
opt_prt_char (const char *o, const char *name, const char *value)
{
...
void
opt_prt_str (const char *o, const char *name, const char *value)
{
    fprintf (stdout, "%3s %-15s: %s\n", o, name, value);
}
/** @brief Print an option and its value in a formatted manner for integer
   ...
   and their integer values in a readable format. */
void
opt_prt_int (const char *o, const char *name, int value)
{
...
void
opt_prt_double (const char *o, const char *name, double value)
{
...
/** @brief Print an option and its value in a formatted manner for boolean
   ...
   based on the value. */
void
opt_prt_bool (const char *o, const char *name, bool value)
{
